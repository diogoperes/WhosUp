...
public class AutocompleteTask extends AsyncTask<String, Void, List<PlaceInfo>> {
    ...
    public interface OnTaskCompleted {
        void onTaskCompleted(List<PlaceInfo> resultList);
    }
    ...
    protected List<PlaceInfo> doInBackground(String... params) {
        ...
        try {
            ...
            for (int i = 0; i < predsJsonArray.length(); i++) {
                ...
                String descriptionFull = place.getString("description");
                String name, description;

                if(descriptionFull.contains(",")) {
                    name = place.getString("description").split(", ", 2)[0];
                    description = place.getString("description").split(", ", 2)[1];
                } else {
